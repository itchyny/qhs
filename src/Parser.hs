...
replaceTableNames qs = (query, tableMap)
  where tableNames = roughlyExtractTableNames qs
        ...
        query = concat [ Map.findWithDefault xs xs tableMap | xs <- split (whenElt isSpace) qs ]
...
roughlyExtractTableNames qs = let qss = words qs in [ ys | (xs, ys) <- zip qss (tail qss), isTableNamePrefix xs ]
  where isTableNamePrefix xs = map toUpper xs `elem` ["FROM", "JOIN"]

-- | Replace the generated table names back into the original file names.
-- This function is used to print the error messages generated by the
-- SQL parser.
replaceBackTableNames :: TableNameMap -> String -> String
replaceBackTableNames tableMap qs = concat [ Map.findWithDefault xs xs reverseMap | xs <- split (whenElt isSpace) qs ]
